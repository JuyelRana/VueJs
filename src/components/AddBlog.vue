<template>
  <div id="add-blog">
    ...
    <form v-if="!submitted">
      ...
      <div id="checkboxes">
        <label>Juyel</label>
        <input type="checkbox" value="Juyel" v-model="blog.categories">
        <label>Rana</label>
        <input type="checkbox" value="Rana" v-model="blog.categories">
        <label>Jalal</label>
        <input type="checkbox" value="Jalal" v-model="blog.categories">
        <label>Mukti</label>
...
export default {
  ...
  methods:{
    post(){
      this.$http.post('https://blog-50e98.firebaseio.com/posts.json', this.blog)
      .then((data)=>{
        console.log(data);
        this.submitted = true;
      });
